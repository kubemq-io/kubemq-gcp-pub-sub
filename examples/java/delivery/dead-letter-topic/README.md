# Java — delivery: Dead-Letter Topic
...
cd examples/java
mvn -q exec:java -Dexec.mainClass=io.kubemq.examples.gcp.delivery.deadlettertopic.Main
```
...
CreateTopic        -> source 'orders-<uuid8>' + dead-letter 'dead-letters-<uuid8>'
CreateSubscription -> sub-worker-<uuid8> (dead_letter_topic='dead-letters-<uuid8>', max_delivery_attempts=5)
CreateSubscription -> sub-dlq-<uuid8> (dead-letter sink)
Publish            -> 'poison' to the source topic
Pull (source)      -> delivery #1 (DeliveryAttempt=1) @ 0s - NOT acking, letting the lease expire
Pull (source)      -> delivery #2 (DeliveryAttempt=2) @ 11s - NOT acking, letting the lease expire
Pull (source)      -> delivery #3 (DeliveryAttempt=3) @ 22s - NOT acking, letting the lease expire
Pull (source)      -> delivery #4 (DeliveryAttempt=4) @ 33s - NOT acking, letting the lease expire
Pull (source)      -> delivery #5 (DeliveryAttempt=5) @ 44s - NOT acking, letting the lease expire
Pull (dead-letter) -> received 'poison' after 5 source deliveries (~56s)

PASS: poison message dead-lettered to 'dead-letters-<uuid8>' once receive count exceeded max_delivery_attempts=5
DeleteSubscription(dlq) -> ok
DeleteSubscription(worker) -> ok
DeleteTopic(dead-letter) -> ok
DeleteTopic(source) -> ok
```
...
- Same variant in the other 5 languages:
  ...
  [csharp](../../../csharp/delivery/dead-letter-topic/),
