...
cd examples/java && mvn -q exec:java -Dexec.mainClass=io.kubemq.examples.gcp.topics.topicmanagement.Main
```
...
GetTopic: labels={stage=demo, team=orders} retention=30m0s
UpdateTopic: labels={stage=production, team=orders}
ListTopicSubscriptions: found sub-managed-<uuid8> (1 total on topic)
DeleteTopic: ok (tombstone — GetTopic=NotFound)
...
project-wide list, `GetTopic` reads its labels and retention back, `UpdateTopic` (an
`UpdateTopicRequest` with a `FieldMask` on `labels`) replaces the label set and asserts the mutation
stuck, and `ListTopicSubscriptions` confirms the subscription is attached. Finally `DeleteTopic`
removes the topic and a follow-up `GetTopic` asserts the delete is a **tombstone** (raises
`NotFound`). The leftover subscription is cleaned up on exit. Any failed assertion throws and the
JVM exits non-zero.

Each topic maps to the Events Store log `pubsub.managed-<uuid8>` and each subscription to the queue
`pubsub.sub.sub-managed-<uuid8>` (spec S3.3). The requested retention is **clamped to the broker's
`Store.MaxRetention`**: `GetTopic` returns the requested value, while fan-out, seek, and the
