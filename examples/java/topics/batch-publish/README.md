...
cd examples/java && mvn -q exec:java -Dexec.mainClass=io.kubemq.examples.gcp.topics.batchpublish.Main
```
...
The program creates a uuid-suffixed topic, then issues N `publish` calls on a single `Publisher`
whose `BatchingSettings` are sized so all N messages coalesce into one server-side batch. Each call
returns an `ApiFuture<String>`; the program collects the server-assigned ids by calling `get` on
each future in the **same order** it issued the publishes, and asserts it got N distinct ids aligned
1:1 to request order. The topic is deleted on exit so re-runs start clean. Any failed assertion
throws and the JVM exits non-zero.
