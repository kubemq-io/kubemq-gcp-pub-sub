package io.kubemq.examples.gcp.advanced.schemaavro;
...
/**
 * advanced: schema-avro — master-table variant 13.
 *
 * <p>Attach an Avro schema to a topic and enforce it on publish:
 * {@code CreateSchema(AVRO) -> CreateTopic{ schema_settings } -> CreateSubscription
 ...
 * Avro is handled connector-side via {@code hamba/avro/v2}; schema definitions are &le; 300 KB
 * with full revision history. JSON-encoded messages must conform to the Avro record.
 *
...
public final class Main {
    ...
    // Avro record { id: long, name: string }. JSON-encoded messages must conform.
    private static final String AVRO_DEFINITION =
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf(
                "[kubemq-gcp-pub-sub] advanced/schema-avro | emulator=%s project=%s%n",
                System.getenv("PUBSUB_EMULATOR_HOST"), projectId);

        // Unique-per-run resource ids (channel isolation). Canonical names: avro-topic / sub-avro.
        String suffix = UUID.randomUUID().toString().substring(0, 8);
        String schemaId = "avro-schema-" + suffix;
        String topicId = "avro-topic-" + suffix;
        String subId = "sub-avro-" + suffix;
        ...
        try (SchemaServiceClient schemaService = SchemaServiceClient.create(
            ...
            // 1. CreateSchema(AVRO) -> BoltDB registry record (<= 300 KB; full revision history).
            schemaService.createSchema(projectName, Schema.newBuilder()
                    .setType(Schema.Type.AVRO)
                    .setDefinition(AVRO_DEFINITION)
                    .build(), schemaId);
            System.out.printf("CreateSchema(Avro) -> %s (<= 300 KB; full revision history)%n",
                    schemaId);
            ...
            try {
                ...
                // 4. Conforming message: JSON matching the Avro record -> accepted.
                ...
                // 5. Non-conforming message: wrong field type (id is a string, not a long) ->
                //    the connector rejects the whole batch with INVALID_ARGUMENT.
                String detail = publishExpectingReject(topicName,
                        "{\"id\":\"not-a-long\",\"name\":\"widget\"}",
                        channelProvider, credentialsProvider);
    ...
    private static boolean matchesSchemaRejection(String detail) {
        ...
        return d.contains("invalid_argument") || d.contains("invalid argument")
                || d.contains("invalid") || d.contains("schema");
    }
