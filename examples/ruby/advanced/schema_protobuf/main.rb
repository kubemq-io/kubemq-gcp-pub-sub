...
# KubeMQ Google Cloud Pub/Sub — advanced: schema validation (Protobuf)
# (Ruby / google-cloud-pubsub).
#
# Demonstrates enforce-on-publish schema validation with a Protobuf schema:
#   CreateSchema(Protobuf) -> CreateTopic(schema_settings → schema, JSON encoding)
...
# Schema definitions are capped at 300 KB. The connector validates Protobuf via
# its protoreflect engine; JSON encoding lets us publish a JSON body that maps to
# the proto message with no generated stubs. The schema type enum on the wire is
# PROTOCOL_BUFFER. See ../../README.md and ../../../SHARED-CONVENTIONS.md.
#
...
# A proto3 message definition: an order with a string id and an int32 quantity.
...
def main
  proj = project_id
  topic_id = rname("proto-topic")
  schema_id = rname("proto-order")
  banner("advanced/schema_protobuf — topic '#{topic_id}'")
  ...
  begin
    # 1) CreateSchema registers the Protobuf definition (≤ 300 KB) with a revision
    #    id. The schema type enum on the wire is PROTOCOL_BUFFER.
    schema = schemas.create_schema parent: pubsub.project_path,
                                   schema_id: schema_id,
                                   schema: { type: :PROTOCOL_BUFFER, definition: PROTO_DEFINITION }
    puts "CreateSchema -> #{schema.name}  type=#{schema.type}  revision=#{schema.revision_id}"
    check(schema.type == :PROTOCOL_BUFFER, "schema registered as PROTOCOL_BUFFER")
    check(!schema.revision_id.empty?, "schema got a server-assigned revision id (revisions kept ≥ 1)")
    ...
    # 3) Conforming publish: a JSON body matching the proto message is accepted
    #    and written once to the topic log pubsub.{topic_id}.
    conforming = { id: "order-2001", quantity: 7 }.to_json
    ok = publisher.publish conforming
    puts "Publish (conforming) -> message_id=#{ok.message_id}"
    check(!ok.message_id.empty?, "conforming JSON publish accepted (matches the Protobuf schema)")

    # 4) Non-conforming publish: 'quantity' is the wrong proto type (a string, not
    #    an int32). The connector validates and rejects the WHOLE batch with
    #    INVALID_ARGUMENT — nothing is enqueued.
    non_conforming = { id: "order-2002", quantity: "seven" }.to_json
    rejected = false
    begin
