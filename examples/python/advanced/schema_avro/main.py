"""KubeMQ Google Cloud Pub/Sub — advanced: Avro schema validation (Python / google-cloud-pubsub).

CreateSchema(Avro) -> CreateTopic(schema_settings, JSON encoding)
...
SCHEMA_ID = f"avro-schema-{SUFFIX}"
TOPIC_ID = f"avro-topic-{SUFFIX}"
SUB_ID = f"sub-avro-{SUFFIX}"

# An Avro record schema (JSON definition) the connector validates each message against.
...
def main() -> None:
    print(f"== advanced/schema_avro — schema '{SCHEMA_ID}', topic '{TOPIC_ID}' ==")
    ...
    try:
        created = schemas.create_schema(
            request={
                ...
                "schema": Schema(type_=Schema.Type.AVRO, definition=AVRO_DEFINITION),
                "schema_id": SCHEMA_ID,
            }
        )
        print(f"CreateSchema(Avro) -> {created.name}  revision={created.revision_id}")
        ...
        conforming = json.dumps({"order_id": "A-1", "amount": 12.5}).encode()
        ...
        non_conforming = json.dumps({"order_id": "A-2", "amount": "not-a-number"}).encode()
        rejected = False
        try:
        ...
        check(
            delivered == ['{"order_id": "A-1", "amount": 12.5}'],
