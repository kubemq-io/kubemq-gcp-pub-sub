"""Enable `python -m advanced.schema_avro` (spec S5.6 module run form).

Loads the sibling main.py by path with run_name="__main__" so the example runs
identically under both `uv run python -m advanced.schema_avro` and
`uv run python advanced/schema_avro/main.py`, without needing package
