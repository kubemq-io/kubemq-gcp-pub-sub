# Python — subscriptions: Ack Deadline & Nack
...
cd examples/python && uv run python -m subscriptions.ack_deadline_nack
```
...
Pull #1            -> body=b'lease-me' ack_id=<ack-id>...
...
Pull #2            -> body=b'lease-me' ack_id=<ack-id>...
...
- One message is published and pulled with a unary `Pull`; it is now **leased** under the
  subscription's ack deadline (10 s) and carries an opaque `ack_id`.
