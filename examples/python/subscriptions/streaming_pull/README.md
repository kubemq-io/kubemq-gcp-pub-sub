# Python — subscriptions: StreamingPull

The high-throughput consume path most client libraries use by default:
`CreateTopic → CreateSubscription → Publish(N) → subscribe() (StreamingPull) → Acknowledge`.

## Prerequisites

- Python 3.10+ and the official GCP Pub/Sub client `google-cloud-pubsub==2.39.0`
  (pinned in `examples/python/pyproject.toml`).
...
cd examples/python && uv run python -m subscriptions.streaming_pull
```
...
StreamingPull      -> stream open (subscribe future); draining via callback
...
- `SubscriberClient.subscribe(...)` opens a long-lived **bidirectional StreamingPull** stream and
  returns a future; the connector pushes messages as they arrive and the callback acks each one.
- An explicit `FlowControl(max_messages=8)` sets the client-side ceiling on outstanding (un-acked)
  messages. The connector also enforces server-side safety nets
  (`MaxOutstandingMessages` 1000, `MaxInflightPerSubscription` 20000).
- The example blocks until all 8 messages arrive, then cancels the stream and asserts each was
  delivered exactly once. The connector periodically closes each StreamingPull stream
...
|--------|-------------|--------------|------------------------|
| CreateTopic, CreateSubscription, Publish ×8, StreamingPull, Acknowledge | `orders-<uuid8>` → `pubsub.orders-<uuid8>` / `sub-orders-<uuid8>` → `pubsub.sub.sub-orders-<uuid8>` | none | `FlowControl(max_messages=8)`; ack deadline 10 s; server ceilings `MaxOutstandingMessages`=1000, `MaxInflightPerSubscription`=20000; stream auto-closes every 1800 s for reconnect |
