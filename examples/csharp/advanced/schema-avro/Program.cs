...
//
// When a topic references a schema, the connector validates every published
// message's `data` against it; validation is part of the atomic batch check, so
// the FIRST non-conforming message rejects the ENTIRE Publish call and nothing in
// that batch is enqueued (spec S3.5 schema).
//
...
// insecure channel credentials (the C# idiom trap for the schema service).
...
var schemaName = SchemaName.FromProjectSchema(projectId, $"avro-order-{suffix}");
var topicName = TopicName.FromProjectTopic(projectId, $"avro-orders-{suffix}");
var subName = SubscriptionName.FromProjectSubscription(projectId, $"avro-sub-{suffix}");

// Avro schema: an order record with a required string id and an int amount.
const string avroDefinition = """
{
  "type": "record",
  "name": "Order",
  "fields": [
...
try
{
    ...
    // CreateSchema(AVRO) — the definition must be valid Avro and ≤ 300 KB.
    var schema = schemas.CreateSchema(new CreateSchemaRequest
    {
        Parent = $"projects/{projectId}",
        SchemaId = schemaName.SchemaId,
        Schema = new Schema { Type = Schema.Types.Type.Avro, Definition = avroDefinition },
    });
    schemaCreated = true;
    Step($"CreateSchema {schemaName.SchemaId} (AVRO, revision {schema.RevisionId})");

    // Bind a topic to the schema with JSON encoding (message `data` is JSON text
    // validated against the Avro schema on every Publish).
    ...
    // an int. Because publish is atomic, the entire batch is rejected and NOTHING
    // is enqueued — not even the valid message.
